...
public class Exer01 {
	...
	public int round10(int num) {
		...
		return (rightMost >= 5) ? num + (10 - rightMost) : num - rightMost;
	}
	
	public int round10version2(int num) {
		int remainder = num % 10;
		num -= remainder;
		return (remainder >= 5) ? num += 10 : num;
	}
	...
	public int blackjack(int a, int b) {
		...
		int aResult = 21 - a;
		int bResult = 21 - b;
		return (aResult > bResult) ? b : a;
	}
	
	//-------------------------------------------------------------------------------------------------------------
	/*
	 * Given three ints, a b c, one of them is small, one is medium and one is large. Return true if 
	 * the three values are evenly spaced, so the difference between small and medium is the same 
